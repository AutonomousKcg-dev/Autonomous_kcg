...
bool autoware_auto_perception_msgs__msg__predicted_object__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("autoware_auto_perception_msgs.msg._predicted_object.PredictedObject", full_classname_dest, 67) == 0);
  }
  autoware_auto_perception_msgs__msg__PredictedObject * ros_message = _ros_message;
  {  // uuid
  ...
  {  // kinematics
    ...
    if (!autoware_auto_perception_msgs__msg__predicted_object_kinematics__convert_from_py(field, &ros_message->kinematics)) {
      Py_DECREF(field);
      return false;
    }
    Py_DECREF(field);
  ...
  {  // shape
    ...
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'shape'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    if (!autoware_auto_perception_msgs__msg__Shape__Sequence__init(&(ros_message->shape), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create autoware_auto_perception_msgs__msg__Shape__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    autoware_auto_perception_msgs__msg__Shape * dest = ros_message->shape.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!autoware_auto_perception_msgs__msg__shape__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of PredictedObject */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("autoware_auto_perception_msgs.msg._predicted_object");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "PredictedObject");
  ...
  {  // kinematics
    PyObject * field = NULL;
    field = autoware_auto_perception_msgs__msg__predicted_object_kinematics__convert_to_py(&ros_message->kinematics);
    if (!field) {
      return NULL;
    }
    {
  ...
  {  // shape
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->shape.data[i]);
      PyObject * pyitem = autoware_auto_perception_msgs__msg__shape__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
