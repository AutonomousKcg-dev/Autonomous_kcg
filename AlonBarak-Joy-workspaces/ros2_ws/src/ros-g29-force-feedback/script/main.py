...
from ros_g29_force_feedback.msg._force_feedback import ForceFeedback
...
cfl = 3  # [m] look forward for a curve
...
def pure_pursuit_steer_control(state, trajectory, pind):
    ...
    # alert alert steering alert alert steering angle alert alert
    degree_for_sus = (2*2.7*math.sin(alpha)/Lf)*(-450)
    # print(trajectory.old_nearest_point_index)

    return degree_for_sus, delta, ind
...
def main():
    ...
    cx = np.array([x for x in x])
    cy = np.array([x for x in y])
    ...
    initial_state = np.array([cx[0],cy[0]])
    ...
    while T >= time and lastIndex > target_ind and not rospy2.is_shutdown():
        ...
        wheel, di, target_ind = pure_pursuit_steer_control(
        ...
        if show_animation:  # pragma: no cover
            ...
            if states.x == 0.0:
                print("see me see me love u alon banana")
